...
export function Users() {
  ...
  const deleteUserMutation = useMutation({
    mutationFn: async (userId) => {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_APP_API_URL}/user/delete-user`,
        {
          method: "DELETE",
          headers: {
  ...
  const { data, error, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await fetch(
        `${
          import.meta.env.VITE_APP_API_URL
        }/user/all?page=${currentPage}&limit=${itemsPerPage}&filter=${order}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
      ...
      () => [
        ...
        // },
        {
          ...
          cell: ({ row }) => (
            <div className="flex items-center gap-2">
              {
                <span className="px-4 py-1 rounded-md opacity-100">
                  {row?.original?.age}
                </span>
              }
            </div>
  ...
  const getPaginationRange = () => {
    ...
    if (endPage - startPage + 1 < maxVisiblePages) {
      return [...Array(maxVisiblePages).keys()].map(
        (i) => i + Math.max(endPage - maxVisiblePages + 1, 1)
      );
    }
